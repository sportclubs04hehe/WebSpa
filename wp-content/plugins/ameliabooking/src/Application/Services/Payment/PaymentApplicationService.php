...
class PaymentApplicationService
{
    ...
    public function createPaymentLink($data, $index = null, $recurringKey = null, $paymentMethod = null)
    {
        ...
        $allAmounts = array_sum(array_column($payments, 'amount'));
        ...
        $amount = $oldPayment['status'] === 'partiallyPaid' ? ($totalPrice - $oldPayment['amount']) : $totalPrice;
        ...
        $methods = $paymentMethod ?: [
            'payPal'   => !empty($entitySettings) ? ($entitySettings['payments']['payPal']['enabled'] && $paymentSettings['payPal']['enabled']) : $paymentSettings['payPal']['enabled'],
            'stripe'   => !empty($entitySettings) ? ($entitySettings['payments']['stripe']['enabled'] && $paymentSettings['stripe']['enabled']) : $paymentSettings['stripe']['enabled'],
            'razorpay' => !empty($entitySettings) ? ($entitySettings['payments']['razorpay']['enabled'] && $paymentSettings['razorpay']['enabled']) : $paymentSettings['razorpay']['enabled'],
            'mollie'   => !empty($entitySettings) ? ($entitySettings['payments']['mollie']['enabled'] && $paymentSettings['mollie']['enabled']) : $paymentSettings['mollie']['enabled'],
        ...
        if (!empty($methods['wc'])) {
            ...
            $amountWithTax = !empty($oldPayment['wcItemTaxValue']) ? ($amount + $oldPayment['wcItemTaxValue']) : $amount;
            if ($oldPayment['status'] === 'partiallyPaid') {
                $linkPayment->setAmount(new Price($amountWithTax));
            ...
            $appointmentData['payment']['newPayment'] = $oldPayment['status'] === 'partiallyPaid';
            ...
            $orderId = !empty($linkPayment->getWcOrderId()) ? $linkPayment->getWcOrderId()->getValue() : null;
            if (empty($orderId)) {
                $orderId = WooCommerceService::createWcOrder($productId, $appointmentData, $amountWithTax, $oldPayment['wcOrderId']);
            }
        ...
        if (!empty($methods['mollie'])) {
            ...
            $paymentData =
                [
                    ...
                    'webhookUrl'  => $callbackLink. '&paymentMethod=mollie',
