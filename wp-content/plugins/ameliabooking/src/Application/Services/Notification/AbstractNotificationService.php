    ...
    private function sendBookingsNotifications($notification, $appointments, $before)
    {
        ...
        foreach ($appointments->toArray() as $appointmentArray) {
            ...
            if ($notification->getSendTo()->getValue() === NotificationSendTo::PROVIDER) {
                ...
                $paymentLinksEnabled = $data['bookable']['settings'] && json_decode($data['bookable']['settings'], true)
                && json_decode($data['bookable']['settings'], true)['payments']['paymentLinks'] ?
                    json_decode($data['bookable']['settings'], true)['payments']['paymentLinks']['enabled'] :
                    $settingsDS->getSetting('payments', 'paymentLinks')['enabled'];
