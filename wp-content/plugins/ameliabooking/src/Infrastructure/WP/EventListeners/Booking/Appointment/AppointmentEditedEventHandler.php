...
class AppointmentEditedEventHandler
{
    ...
    public static function handle($commandResult, $container)
    {
        ...
        if ($googleCalendarService && !$appointmentEmployeeChanged && $reservationObject->getProvider()) {
            try {
        ...
        if ($outlookCalendarService && !$appointmentEmployeeChanged && $reservationObject->getProvider()) {
            try {
        ...
        $paymentLinksEnabled = $reservationObject->getService()->getSettings() && json_decode($reservationObject->getService()->getSettings()->getValue(), true)
        && json_decode($reservationObject->getService()->getSettings()->getValue(), true)['payments']['paymentLinks'] ?
            json_decode($reservationObject->getService()->getSettings()->getValue(), true)['payments']['paymentLinks']['enabled'] :
            $settingsService->getSetting('payments', 'paymentLinks')['enabled'];


        if ($paymentLinksEnabled) {
            foreach ($appointment['bookings'] as $index => $booking) {
                $newPrice = $paymentAS->calculateAppointmentPrice($booking, Entities::APPOINTMENT);
                if (array_sum(array_column($booking['payments'], 'amount')) < $newPrice) {
        ...
        if ($whatsAppNotificationService && $whatsAppNotificationService->checkRequiredFields()) {
