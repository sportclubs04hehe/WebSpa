    ...
    methods: {
      getPaymentsStatus (payment) {
        ...
        let booking = {
          ...
          price: payment.bookedPrice + payment.bookingExtrasSum,
          aggregatedPrice: payment.aggregatedPrice,
          persons: payment.persons
        }
        let bookingPrice = this.getBookingPrice(booking, true, booking.price, booking.aggregatedPrice)
        bookingPrice += allPayments.filter(p => p.wcOrderId && p.wcItemTaxValue).reduce((partialSum, a) => partialSum + a.wcItemTaxValue, 0)
        let paidAmount = allPayments.filter(p => p.status !== 'pending').reduce((partialSum, a) => partialSum + a.amount, 0)
        if (paidAmount >= bookingPrice) {
          return 'paid'
        }
        return allPayments.filter(p => p.status === 'partiallyPaid').length > 0 ? 'partiallyPaid' : 'pending'
      },
